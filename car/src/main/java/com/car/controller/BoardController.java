...
public class BoardController {
	...
	public String updateBoard(HttpServletRequest req) {
		...
		// 3. 목록 페이지로 이동
		return "redirect:/board/detail.action" + "?boardno=" + Board.getBoardNo() + "&pageno="
	...
	public ModelAndView showReviewByBoardNo(HttpServletRequest request) {
		...
		mav.addObject("pageno", pageNo);
		mav.addObject("boardupload", boardupload);
		return mav;
	...
	public String updateReview(HttpServletRequest req) {
		...
		// 3. 목록 페이지로 이동
		return "redirect:/board/detail2.action" + "?boardno=" + Board.getBoardNo() + "&pageno="
				+ req.getParameter("pageno");
	}
	...
	public String writeComent(HttpServletRequest req) {
		...
		coment.setWriter(((Member) req.getSession().getAttribute("loginuser")).getMemberId());
	...
	public String deleteComent(HttpServletRequest req) {
		...
		// 3. view.action로 이동
		return "redirect:/board/detail2.action?" + "boardno=" + req.getParameter("boardNo") + "&pageno="
				+ req.getParameter("pageno");
	...
	public String updateComent(HttpServletRequest req) {
		...
		BoardComent coment = new BoardComent();
		coment.setComentNo(Integer.parseInt(req.getParameter("comentNo")));
		coment.setContent(req.getParameter("content"));
